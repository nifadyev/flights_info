...
class TestValidateCityCodes(unittest.TestCase):
    ...
    def setUp(self):
        self.valid_codes = frozenset(('CPH', 'BLL', 'PDV', 'BOJ', 'SOF', 'VAR'))
    ...
    def test_equal_dep_time_and_arr_time(self):
        ...
        for time in args:
            with self.subTest(time):
                self.assertEqual(source.calculate_flight_duration(time, time), '00:00')


class TestCheckRoute(unittest.TestCase):

    def setUp(self):
        self.city_codes = frozenset(('CPH', 'BLL', 'PDV', 'BOJ', 'SOF', 'VAR'))
        self.dates = {
            ('CPH', 'BOJ'): frozenset((
...
class TestParseArguments(unittest.TestCase):
    ...
    def test_too_many_args(self):
        args = ('CPH', 'B0J', '01.07.2019', '3', '-return_date=03.08.2019', '0')
